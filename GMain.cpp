...
public:
    ...
    void insert(string seq, string qual) {
        ...
        for (i = 0; i < SIZE - K + 1; i++) {
            ...
            if (isVariation(kmer) && quality(kmer_qual)) {  //&& quality(kmer_qual)
    ...
    void errorPrune() {
        ...
        while (vmap_it_t != vmap.end()) {
            ...
            if (vcount == 1) {
                cerr << "****" << vmap_it_t->first.to_string() << endl;
...
int main() {
    ...
    kG.insert(s);
    varGraph vG(&kG);
    vG.fq_read("NWD143123.recab.chr22.1M.fastq");
    vG.storeToFile("1M_Variant.txt");
    return 0;
}
